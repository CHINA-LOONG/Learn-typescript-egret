...
class TriangleUtil {
	...
	/**
	 * 求外接圆半径 https://baike.baidu.com/item/外接圆半径公式/14818938?fr=aladdin
	 * @param tri
	 * @return
	 */
	public static getCircumcircleR(tri: Tri2D): number {
		let x1: number = tri.vertex[0].x;
		let x2: number = tri.vertex[1].x;
		let x3: number = tri.vertex[2].x;
		let y1: number = tri.vertex[0].y;
		let y2: number = tri.vertex[1].y;
		let y3: number = tri.vertex[2].y;
